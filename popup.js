...
chrome.tabs.query({ active: true, currentWindow: true }, async ([tab]) => {
  if (!tab?.id || !/^https?:\/\//.test(tab.url) || /\/admin(\/|$)/.test(tab.url)) {
    ...
    const [result] = await chrome.scripting.executeScript({
      ...
      func: async () => {
        ...
        const getHandle = () => {
          ...
          return m?.[1] || window.ShopifyAnalytics?.meta?.product?.handle || null;
        };
        ...
        const isVariantPurchasable = (variant) =>
          ...
          (Array.isArray(variant?.selling_plan_allocations) && variant.selling_plan_allocations.length > 0);
        ...
        const collectProductSoldOut = async () => {
          ...
          if (productJson?.variants?.length) {
            const nonePurchasable = productJson.variants.every(v => !isVariantPurchasable(v));
